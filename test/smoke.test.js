...
`);
  ...
  const result = spawnSync(process.execPath, [cli], { cwd: sandbox, env: agentEnv, input: 'Create a tiny feature\n/exit\n', encoding: 'utf8' });
  assert.equal(result.status, 0, result.stderr);
...
`);
  ...
  const result = spawnSync(process.execPath, [cli], { cwd: sandbox, input: 'Answer a question\n/exit\n', encoding: 'utf8' });
  assert.equal(result.status, 0, result.stderr);
...
test('supports resume, permissions, and model controls inside duet', () => {
  const sandbox = fs.mkdtempSync(path.join(os.tmpdir(), 'duetai-controls-'));
  const claude = path.join(sandbox, 'mock-claude.js');
  const codex = path.join(sandbox, 'mock-codex.js');
  fs.writeFileSync(claude, `#!/usr/bin/env node
require('node:fs').appendFileSync('claude-args.jsonl', JSON.stringify(process.argv.slice(2)) + '\\n');
const review = process.argv.join(' ').includes('meticulous senior reviewer');
const text = review ? 'VERDICT: PASS\\nFINDINGS: none' : 'MODE: DUET\\nPLAN\\n1. Implement it.';
console.log(JSON.stringify({type:'assistant',message:{content:[{type:'text',text}]}}));
`);
  fs.writeFileSync(codex, `#!/usr/bin/env node
require('node:fs').appendFileSync('codex-args.jsonl', JSON.stringify(process.argv.slice(2)) + '\\n');
console.log(JSON.stringify({type:'item.completed',item:{type:'agent_message',text:'DECISION: done\\nCHANGED: none\\nVALIDATION: complete'}}));
`);
  fs.chmodSync(claude, 0o755); fs.chmodSync(codex, 0o755);
  fs.writeFileSync(path.join(sandbox, '.duet.json'), JSON.stringify({
    claude: { command: claude }, codex: { command: codex }, workflow: { maxRounds: 1 }
